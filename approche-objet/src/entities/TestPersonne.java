package entities;

public class TestPersonne {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
		Personne personne = new Personne("Andrade Sanchez","Olivier",new AdressePostale(int numeroDeRue, String libelleDeRue, int codePostal, String ville));
		personne.nom="Andrade Sanchez";
		personne.prenom="Olivier";
		personne.adressePostale=new AdressePostale();
		
		Personne personne2 = new Personne();
		
		
		
		//AdressePostale adresse = new AdressePostale(12, "rue du commerce", 34000 , "MONTPELLIER");
		
	
		
		//Personne alex = new Personne("Alex","DUPONT",adresse);
		
		//System.out.println(alex.adressePostale.codePostal);
		
		//Personne michel = new Personne();
		
	}

}
